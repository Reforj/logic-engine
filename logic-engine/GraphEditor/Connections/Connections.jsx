...
const Connection = ({node, pin, offset, onRemove, scrollTop, scrollLeft, zoom}) => {
  const path = (srcPin, destPin, index) => {
    ...
    return <path key={destPin.socket} onClick={click} id="path" className={cs(css.line, css[pin.exec ? 'exec' : 'call'], css[pin.dataType])} d={d} fill="none"/>
  }
...
const TempConnection = ({temp: {begin: b, end: e, offset, right, type, dataType}}) => {
...
export default function Connections ({nodes, offset, temp, removeConnection, ui, scrollTop=0, scrollLeft=0, zoom}) {
  ...
  return <svg style={{width: '100%', height: '100%'}} xmlns="http://www.w3.org/2000/svg" >
    ...
    {offset && _.map(connections, (c) => {
      return _.map(c.pins, (p) => <Connection
        node={c.node}
        key={p.uuid}
        pin={p}
        offset={offset}
